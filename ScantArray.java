...
public class ScantArray {
    ...
    public int getValueAt(int row, int col){
        for(ScantArrayEntry a: entries){
            if(a.getRow()==row&&a.getColumn()==col) return a.getValue();
        }
        return 0;
    }
    ...
    public void removeColumn(int col){
        ...
        for(int i=0; i<entries.size();i++){
            ...
            entries.remove(i);
            if(a.getColumn()>col){
                entries.add(new ScantArrayEntry(a.getRow(),a.getColumn()-1,a.getValue()));
            if(a.getColumn()<col){
                entries.add(new ScantArrayEntry(a.getRow(),a.getColumn(),a.getValue()));
            }
        }
        }
